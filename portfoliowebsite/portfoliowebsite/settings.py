...
if os.environ.get('WEBAPP_HOSTNAME'):
    ALLOWED_HOSTS.append(os.environ['WEBAPP_HOSTNAME'])
...
if os.environ.get('WEBAPP_HOSTNAME'):
    CSRF_TRUSTED_ORIGINS.append(f'https://{os.environ["WEBAPP_HOSTNAME"]}')
...
MIDDLEWARE = [
    ...
    'portfoliowebsite.whitenoise.MediaWhiteNoiseMiddleware',
...
# User uploads (profile picture, resume) managed from the Django admin.
MEDIA_URL = 'media/'
...
# Security for production (Render terminates HTTPS at its proxy)
if not DEBUG:
...
# Email
# Contact-form messages are logged to the server console by default. To receive
# real emails, set the EMAIL_* environment variables on your hosting provider.
